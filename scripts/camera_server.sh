...
gst-launch-1.0 nvarguscamerasrc sensor-id=0 ! 'video/x-raw(memory:NVMM),fromat=NV12,width=1280,height=720,framerate=60/1' ! m.sink_0 \
    ...
    ! nvvideoconvert ! 'video/x-raw(memory:NVMM),width=640,height=720' ! tee name=t ! queue ! nv3dsink sync=false \
    t.src_1 ! queue ! nvvidconv ! 'video/x-raw,width=640,height=720' ! jpegenc ! rtpjpegpay ! udpsink host=192.168.3.9 port=8554 sync=false
